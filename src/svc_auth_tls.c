...
bool svcauth_tls_send_response(struct svc_req *req)
{
    ...
    memcpy(req->rq_msg.RPCM_ack.ar_verf.oa_body, starttls_token, STARTTLS_TOKEN_LEN);
    ...
    if (rc >= XPRT_DIED) {
         LogDebugTLS(DPP_HANDSHAKE, "svc_sendreply failed for AUTH_TLS response on fd %d", xprt->xp_fd);
         res = false;
    } else {
         LogDebugTLS(DPP_HANDSHAKE, " AUTH_TLS STARTTLS response sent on fd %d", xprt->xp_fd);
    }
...
enum auth_stat svcauth_tls_checks(struct svc_req *req)
{
        ...
        if (req->rq_msg.cb_cred.oa_length != 0) {
                ...
                LogDebugTLS(DPP_HANDSHAKE,
                         "AUTH_TLS credential length must be zero, got %u",
        ...
        if (req->rq_msg.cb_cred.oa_flavor != AUTH_TLS) {
                LogDebugTLS(DPP_HANDSHAKE, "Expected AUTH_TLS flavor, got %d",
        ...
        if (req->rq_xprt->xp_tls.tls_enabled || req->rq_xprt->xp_tls.tls_established) {
                LogDebugTLS(DPP_HANDSHAKE,
                        "AUTH_TLS probe within existing TLS session on fd %d",
                         req->rq_xprt->xp_fd);
                rc = AUTH_BADCRED;
                return rc;
        }
        ...
        if (req->rq_msg.cb_proc != NULLPROC) {
                LogDebugTLS(DPP_HANDSHAKE,
                         " AUTH_TLS only allowed on NULL procedure, proc %u",
                         req->rq_msg.cb_proc);
                rc = AUTH_BADCRED;
        }
...
enum auth_stat _svcauth_tls(struct svc_req *req)
{
	...
	if (rslt == AUTH_OK) {
                 LogDebugTLS(DPP_HANDSHAKE,
                        "TLS Valid AUTH_TLS probe detected on fd %d",
                         req->rq_xprt->xp_fd);
